    ...
    switch ($method) {
        case 'GET':
            if ($resourceId) {
                if ($subAction === 'subjects') {
                    $sql = "SELECT mss.MSS_ID, mss.MSS_STU_DOW, mss.MSS_STU_TIME, sub.SUB_ID, sub.SUB_NAME, sub.SUB_CLASS, sub.SUB_BOARD, sub.SUB_LANG, tchr.TCHR_ID, tchr.TCHR_NM, tchr.TCHR_EMAILID, tchr.TCHR_CONTACTNO 
                            FROM MAP_STU_SUB mss
                            JOIN MAP_TCHR_SUB mts ON mss.MSS_STU_TCHR = mts.MTS_ID
                            JOIN MSTR_SUB sub ON mts.MTS_SUB = sub.SUB_ID
                            JOIN MSTR_TEACHER tchr ON mts.MTS_TCHR_ID = tchr.TCHR_ID
                            WHERE mss.MSS_STU_ID = ?";
        ...
        case 'POST':
            ...
            try {
                ...
                $userEmail = $data['STU_EMAILID'] ?? '';
                $userNm = $data['STU_NM'] ?? '';
                $userId = $data['STU_ID'] ?? '';
                if ($userEmail && $userId) {
                    $userStmt = $pdo->prepare("INSERT INTO MSTR_USER (USERNAME, PASSWORD, ROLE, REF_ID, EMAIL, NAME) VALUES (?, ?, 'Student', ?, ?, ?)");
                    $userStmt->execute([
                        $userId,
                        password_hash('student@123', PASSWORD_DEFAULT),
                        $userId,
                        $userEmail,
                        $userNm
                    ]);
        ...
        case 'PUT':
            ...
            try {
                ...
                // Sync credentials
                $userStmt = $pdo->prepare("UPDATE MSTR_USER SET EMAIL = ?, NAME = ? WHERE REF_ID = ? AND ROLE = 'Student'");
                $userStmt->execute([
                    $data['STU_EMAILID'],
                    $data['STU_NM'],
                    $resourceId
        ...
        case 'DELETE':
            ...
            // Sync user deletion
            $userStmt = $pdo->prepare("DELETE FROM MSTR_USER WHERE REF_ID = ? AND ROLE = 'Student'");
